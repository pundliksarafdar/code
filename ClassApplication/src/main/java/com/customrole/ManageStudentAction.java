...
public class ManageStudentAction extends BaseAction{
	...
	public String performBaseAction(UserBean userBean,HttpServletRequest request,HttpServletResponse response,Map<String, Object> session) {
		...
		List list=batchTransactions.getAllBatches(userBean.getInst_id());
		CertificateTransaction certificateTransaction = new CertificateTransaction();
		certificateList = certificateTransaction.getCertificateTemplateList(userBean.getInst_id());
		request.setAttribute("batches", list);
			studentHelperBean.setClass_id(userBean.getInst_id());
