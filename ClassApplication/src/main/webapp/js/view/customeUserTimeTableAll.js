...
var getTimetable = "rest/customuserservice/getScheduleForMonth/";
var saveScheduleUrl = "rest/customuserservice/schedule";
var old_date;
var timtableData,
	calendar,
	validatorForm;
...
$(document).ready(function(){
	...
	if($.inArray( "23", child_mod_access) == "-1"){
		canEdit = false;
		}
		if($.inArray( "24", child_mod_access) == "-1"){
			canDelete = false;
		}
	...
	$(SUBJECT_SELECT).empty();
	  ...
	  if(batchName != "-1" && batchName!=null && batchName != ""){
	   var handler = {};
		...
		rest.get("rest/customuserservice/getBatchSubjects"+"/"+$(DIVISION_SELECT).val()+"/"+$(BATCH_SELECT).val(),handler);
	  }else{
		  	var options = "<option value='-1'>Select Subject</option>";
		  	$(SUBJECT_SELECT).append(options);
			$(SUBJECT_SELECT).select2().val("-1").change();
	  ...
	  if(subname != "-1" && subname != null && subname != ""){
		  ...
		  $.ajax({
			   url: "classOwnerServlet",
			    data: {
			    	 methodToCall: "customUserFetchSubjectTeacher",
			    	 subname:subname
			   		}, 
			   type:"POST",
