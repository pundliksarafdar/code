...
$(document).ready(function(){
	...
	$("#division").change(function(){
		...
		if(division != "-1"){
			...
			rest.get("rest/customuserservice/getSubjectOfDivisionForExam/"+division,handlers);
		}else{
			$("#subject").empty();
			 $("#subject").select2().val("").change();
	...
	$(SAVE_QUESTION_PAPER).on("click",function(){
		...
		if(desc && desc.trim().length!==0){
			...
			rest.post("rest/customuserservice/updateQuestionPaper/"+patternId+"/"+questionPaperName+"/"+division+"/"+paperId+"/"+compSubjectIds,handlers,JSON.stringify(questionPaperData),true);	
		}else{
	...
	$("body").on("click",SEARCH_QUESTION_PAPER,searchQuestionPaper).
		...
		$("body").on("click",REGENERATE_QUE_BTN,function(e){
			...
			var url = "rest/customuserservice/generateQuestionPaperSpecific/"+$("#division").val();
			var handler = {};
		...
		var url = "rest/customuserservice/generateQuestionPaper/"+$("#division").val();
		var handler = {};
		handler.success = function(e){
		...
		rest.post(url,handler,JSON.stringify(generateExamObjectList));
			...
			$("#subject").change(function(){
				...
				var division = $("#division").val();
				rest.get("rest/customuserservice/getSubjectsAndTopicsForExam/"+division+"/"+subject,handlers);
			});
});
...
var searchQuestionPaperSuccess = function(data){
	...
	{
		title: "Marks",data:'marks',sWidth:"10%"
	},
	{
		title: "Actions",data:null,sWidth:"20%",render:function(){
			var buttons = "<div>";
			if($.inArray( "29", child_mod_access) != "-1"){
				buttons = buttons + "<a class='btn btn-primary btn-xs editQuestionPaper'>Edit</a>&nbsp;";
			}
			if($.inArray( "30", child_mod_access) != "-1"){
				buttons = buttons + "<a class='btn btn-danger btn-xs deleteQuestionPaper'>Delete</a>";
			}
			buttons = buttons + "</div>"
			return buttons },bSortable:false
	}]});
}
	...
	if(data.length > 0){
		for(var i=0; i<data.length; i++){
			...
			if(itemType == "Section"){
				...
				if(data[i].questionbank.que_type == "3"){
					question = $("<div/>",{
						class:"col-md-5 question",
						html:data[i].questionbank?data[i].paragraphQuestion.paragraphText:"<span class='error'>Question is not available</span>"
					}).data(QUESTION_ID,(data[i].questionbank?data[i].questionbank.que_id:undefined));
				}
...
var loadQuestionList = function(subject_id,item_id,question_type,item_type,question_topic,item_marks){
			...
			var url = "rest/customuserservice/getQuestionList/"+$("#division").val();
			var handler = {};
