...
$(document).ready(function(){
	...
	/* $("#divisionSelect").on("change",function(e){
	 ...
	 $("#divisionSelect").on("change",function(e){
			...
			if($(this).val()!=-1){
				...
				$("#subjectSelect").html("");
				$("#subjectSelect").select2({data:"",placeholder:"Select Subject"});
				$("#batchError").html("");
				$("#batch").select2("val", "");
				$("#batch").html("");
				$("#batch").select2({data:"",placeholder:"Select Batch"});
			}
		});
	...
	$("#subjectSelect").on("change",function(e){
		...
		if(subjectId!="-1"){
			...
			$("#batchError").html("");
			$("#batch").select2({data:batchArray,placeholder:"type batch name"});
			$("#batch").select2("val", "");
	...
	handlers.success=function(data){
		 ...
		 var selectOptionDropdown = "#subjectSelect";
			...
			$("<option/>",{
				...
				$("#batchError").html("");
				$("#batch").html("");
				$("#batch").select2({data:batchDataArray,placeholder:"Select Batch"});
				$("#batch").select2("val", "");
...
function UploadExam(){
	...
	var saveSyllabus = function(){
		...
		rest.post("rest/customuserservice/savePlannedSyllabus",handler,JSON.stringify(sBean),true);
	}
	...
	var onDateChange = function(){
		...
		rest.get("rest/customuserservice/getSyllabusForMonth"+"/"+date+"/"+$("#divisionSelect").val()+"/"+$("#subjectSelect").val()+"/"+$("#batch").val(),handler);
	}
	...
	var displayBatchFromSubjectNDivision = function (data){
		...
		var tempBatchData= {};
		tempBatchData.id = "-1";
		tempBatchData.text = "Select Batch";
		batchDataArray.push(tempBatchData);
	...
	function editSyllabus(data,syllabusText,date,teacherStatus){
		...
		rest.put("rest/customuserservice/savePlannedSyllabus",handler,JSON.stringify(data));
	}
	...
	function deleteRecord(data){
		...
		rest.deleteItemData("rest/customuserservice/deletePlannedSyllabus",handler,JSON.stringify(data));
	}
		...
		oTable  = $(TEACHER_SYLLABUS_TABLE).DataTable( {
			...
			},
			{
				title: "",bSortable:false,data:null,render:function(data){if(data.editable){return "<input type='button' class='btn btn-success btn-xs edit' value='Edit'/>&nbsp;" +
						"<input type='button' class='btn btn-danger btn-xs delete' value='Delete'/>&nbsp;"}else{
