	...
	var dataTable = $('#patternListTable').DataTable({
		...
		columns: [{title:"#",data:null,render:function(data,event,row){
			...
			{ title: "Edit",data:null,render:function(data,event,row){
				...
				if($.inArray( "26", child_mod_access) != "-1"){
					buttons = buttons + '<input type="button" class="btn btn-xs btn-primary viewPattern" value="Edit" id="'+row.pattern_id+'">';
				}
				buttons = buttons + '</div>';
				return buttons;
			},sWidth:"10%"},
			{ title: "Delete",data:null,render:function(data,event,row){
				var buttons = '<div class="default">';
				if($.inArray( "27", child_mod_access) != "-1"){
					buttons = buttons + '<input type="button" class="btn btn-xs btn-danger deletePattern" value="Delete" id="'+row.pattern_id+'">';
				}
				buttons = buttons + '</div>'
				return buttons;
			},sWidth:"10%"}
...
$(document).ready(function(){
	...
	$("#searchPattern").click(function(){
		...
		handlers.error = function(e){$.notify({message: "Error"},{type: 'danger'});};
		rest.get("rest/customuserservice/searchQuestionPaperPattern/"+division+"/"+subject+"/"+patternType,handlers);
		}
	});
	...
	$("#patternListTable").on("click",".viewPattern",function(){
		...
		handlers.error = function(e){
			$.notify({message: "Error"},{type: 'danger'});}
		editString = "";
		rest.get("rest/customuserservice/getQuestionPaperPattern/"+division+"/"+patternid,handlers);
	});
	...
	$("#patternListTable").on("click",".deletePattern",function(){
		...
		rest.deleteItem("rest/customuserservice/deleteQuestionPaperPattern/"+division+"/"+patternid,handlers);
	});
...
function getTopics(subID,that,scenario){
	...
	if(subID != ""){
	 $.ajax({
		   ...
		   data: {
		    	 methodToCall: "customUsergetDivisionsTopics",
		    	 divisionID:division,
		    	 subID:subID
		   		},
		   type:"POST",
...
function generatePattern(){
	...
	rest.post("rest/customuserservice/updateQuestionPaperPattern",handlers,QuestionPaperPatternJson,false);
	}
}
...
<body>
	<jsp:include page="ExamHeader.jsp" >
		<jsp:param value="active" name="customeUserViewnEditPattern"/>
	</jsp:include>
