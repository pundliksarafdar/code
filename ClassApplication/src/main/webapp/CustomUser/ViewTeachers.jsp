	...
	*/
	...
	function deleteteacher(divisionId,teacherIdToDelete){		
				...
				handlers.error=function(){
					$.notify({message: "teacher not deleted"},{type: 'danger'});
				};   
				rest.deleteItem("rest/customuserservice/deleteTeacher/"+teacherIdToDelete,handlers);
	}
	...
	function saveteacherAjax(teacherId,teacherSuffix,subjectIds){
		...
		teacher.suffix = teacherSuffix;
		teacher.sub_ids =subjectIds;
		 ...
		 var handlers = {};
			handlers.success=function(data){
				 if(data){
				   		...
				   		$.notify({message: "Teacher not updated Successfully."},{type: 'danger'});
				   		 }
			};   
			handlers.error=function(){};   
			
			rest.put("rest/customuserservice/updateTeacher/",handlers,JSON.stringify(teacher));
		}
	}
	...
	function getAllteacheres(){
		...
		rest.get("rest/customuserservice/getAllTeachers/",handlers);
	}
