...
var enabledEdit = false;
	$(document).ready(function(){
		...
		$("#searchStudentByName").click(function(){
			...
			rest.get("rest/customuserservice/getStudentByName/"+studentName,handlers);
			 ...
			 var handlers = {};
				handlers.success=function(data){
					...
					var batchDataArray = [];
					 ...
					 that.closest("tr").find(".selectBatch").select2({data:batchDataArray,placeholder:"type batch name"});
					}else{
	...
	function studentDetails(){
		...
		rest.get("rest/customuserservice/getStudentDetails/"+studentId,handlers);
	}
	function enableEdit(){
		enabledEdit = true;
	...
	function deleteStudent(studentId,that){
		...
		rest.deleteItem("rest/customuserservice/deleteStudent/"+studentId,handlers);
	}
	...
	function successCallbackclass(data,type){
		...
		if(type == "name"){
			dataTable = $('#classTable').DataTable({
				...
				columns: [
					...
					{ title: "Actions",data:null,render:function(data){
						...
						if($.inArray( "11", child_mod_access) != "-1"){
							buttons = buttons + '<input type="button" class="btn btn-xs btn-primary btn-batch-edit" value="Edit">&nbsp;';
						}if($.inArray( "12", child_mod_access) != "-1"){
							buttons = buttons + '<input type="button" class="btn btn-xs btn-danger btn-batch-delete" value="Delete">&nbsp;';
						}
						buttons = buttons + '<input type="button" class="btn btn-xs btn-info btn-student-details" value="Details">&nbsp;';
						if($.inArray( "51", child_mod_access) != "-1"){
						buttons = buttons +	'<input type="button" class="btn btn-xs btn-primary btn-certificate-print" value="Print Certificate">&nbsp;';
						}
						buttons = buttons +'</div>'+
					'<div class="editable">'+
			...
			if(data.length>0){		
				$(".generateRollNumber").removeClass('hide');
				}
		dataTable = $('#classTable').DataTable({
			language: {
			        "emptyTable":     "Students not available"
					...
					if(data.length > 0){
					 $.each(data,function(key,val){
					    ...
					    $("#batch").select2({data:batchDataArray,placeholder:"type batch name"}).data("batchData",batchData);
				    	 ...
				    	 $("#batch").select2({data:null,placeholder:"Batch not found"});
				    }
				}
				handlers.error = function(e){}
				rest.get("rest/customuserservice/getBatches/"+divisionId,handlers);
	...
	function getNames(){
		...
		handlers.error = function(e){}
		rest.get("rest/customuserservice/getNamesForSuggestion/",handlers,true,false);
	}
	...
	function getAllClasses(that){
		...
		rest.get("rest/customuserservice/getAllClasses",handlers);
